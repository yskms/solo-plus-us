...
- **日時編集 UI**：§4.4/§11.4「過去日時への記録・編集」は要件定義書 §25 の MVP 表で v1.0 必須（●）
  とされているが、基本設計 §18 の Phase 1〜4 のどこにも明記されていなかった（このレビューで指摘）。
  **Phase 3 に割り当てることを決定**（Phase 1 の Known gaps を参照）
