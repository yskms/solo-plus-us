...
- **日時編集 UI の担当 Phase が未確定**：§4.4/§11.4「過去日時への記録・編集」は要件定義書 §25 の
  MVP 表で v1.0 必須（●）とされているが、基本設計 §18 の Phase 1〜4 のどこにも明記されておらず、
  このまま繰り越すと抜け落ちる懸念がある。どの Phase で実装するかはユーザーと要相談（ネイティブの
  日時ピッカーが必要で、実機ビルドが通ることが前提になる点も含めて）
