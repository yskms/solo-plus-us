...
作成日：2026-09-16  
前提：要件定義書 v0.8 / 設計判断記録 v0.8  
v0.1 からの変更点と理由は **設計判断記録 v0.8** を参照する。
...
| 順 | 条件 | 処理 |
|---:|---|---|
| 1 | mapping あり + `update` / `recreate` ジョブ | **delete ジョブへ置換** → mapping 削除 → Activity 削除 |
| 2 | mapping あり + ジョブなし | delete ジョブ作成 → mapping 削除 → Activity 削除 |
| 3 | mapping なし + `create`、`attempts > 0` または claim 中 | **delete ジョブへ置換** + Activity 削除 |
| 4 | mapping なし + `create`、`attempts = 0` かつ未 claim | **create ジョブを削除**して Activity 削除 |
| 5 | mapping なし + ジョブなし + 現在も同期対象外 | Activity のみ削除 |

`attempts > 0` の行（順3）が要点である。これがないと「送信済みの create を黙って取り消す」
経路が残り、ローカルに存在しない記録が Health Connect 側に残る。
...
**§10.1 と同じ判定順序を使う。「HC が未接続かどうか」を先に見ない。**

| 順 | 条件 | 処理 |
|---:|---|---|
| 1 | mapping あり | delete ジョブを作る |
| 2 | `update` / `recreate` ジョブ | **delete ジョブへ置換** |
| 3 | `create` ジョブ、`attempts > 0` | **delete ジョブへ置換** |
| 4 | `create` ジョブ、`attempts = 0` | **ジョブごと削除**（外部へ未到達が確実） |
| 5 | mapping なし + ジョブなし + 現在も同期対象外 | Activity のみ削除 |
