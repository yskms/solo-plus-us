...
作成日：2026-09-15（v0.11 改訂：2026-09-16）  
ステータス：実装着手前の確定版  
関連：基本設計 v0.11 / UI-UX Specification v0.11 / **設計判断記録 v0.11**
...
- **切断中はワーカーが Health Connect のジョブを実行しない**（再接続まで保持する）
...
| Calendar（月表示・日別一覧） | ● | | |
| Insights（合計・内訳・平均間隔、全期間） | ● | | タブは v1.0 から3つ出す。期間セレクタは無く、常に全期間（All Time）集計 |
| Insights（期間セレクタ・月次棒グラフ・曜日・時間帯） | | ● | Month/Year の期間セレクタ、月次棒グラフ、最頻曜日・最頻時間帯 |
| ローカル SQLite | ● | | |
