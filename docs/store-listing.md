...
| 広告 | 広告を含まない | |
| 配信国 | 上記14か国を除く全世界 | 下記参照 |

### 配信国の方針（2026-09-24 決定）

**「できるだけ多くの国に出す。ただし、性的な内容を含むアプリの規制が厳しいと
される地域は外す」**という方針で、以下の14か国を除外して公開した。

サウジアラビア / アラブ首長国連邦 / カタール / クウェート / バーレーン /
オマーン / イエメン / イラン / パキスタン / バングラデシュ / モルディブ /
エジプト / スーダン / リビア

判断の前提（過度に恐れないための整理）：

- 現実的に起きうるのは**その国での配信停止**であって、開発者個人が法的責任を
  問われる事態ではない（国単位の措置で、アカウント全体には通常波及しない）
- 利用者側のリスクは国によって異なる——婚姻外/同性間の行為が処罰対象の国では、
  端末にアプリが入っていること自体が不利に働きうる。App Lock や Recent Apps
  プレビュー非表示は、こうした状況も想定した機能である
- インドネシア / マレーシア / ナイジェリア / ウガンダは「判断が分かれる」候補
  として挙げたが、今回は除外していない
- **配信国は後から変更できる。** 問題が起きなければ広げ、逆に指摘があれば狭める
- なお、キューバ・イラン・ロシア・ベラルーシ等は米国輸出規制や Google 側の
  措置により、選択しても実際には配信されない場合がある
