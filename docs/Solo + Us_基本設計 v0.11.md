...
### 削除に失敗した場合（2026-09-21 改訂——設計判断記録 D-34「§13.6 実装時の結論」参照）

**delete の失敗が `UNKNOWN` 分類（存在しないレコードへの削除など）の場合は、
insert へ進む。** `PERMISSION_DENIED`/`UNAVAILABLE` は従来通り作成しない
——ジョブはリトライに回り、上限到達で手動待ちになる（§9.6）。

当初は「delete に失敗したら常に作成しない（外部に同じ記録が二重に存在
する状態を作らない）」としていたが、この理由は provider が外部発行 ID
でアドレッシングする場合（将来の HealthKit 等）を想定したジェネリックな
安全策であり、v1 が実装する唯一の provider（health_connect）には
当てはまらないことが実装時に判明した。health_connect は `clientRecordId`
（§9.4、クライアント側で決定的に発行）でアドレッシングするため、delete
が失敗していても insert は新規レコードを作らず、HC 側が version の大小で
自動的に処理する upsert として振る舞う——二重レコードという懸念は
構造的に発生しない。

§13.6 の主要ユースケース（機種変更・復旧）では復元先の Health Connect に
対象レコードが1件も存在しないため、当初の「常に作成しない」では
Android 9〜13（D-20）で recreate が全件・恒久的に失敗していた。詳細・
残存リスクの評価は設計判断記録 D-34 参照。
