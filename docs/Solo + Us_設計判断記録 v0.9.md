...
作成日：2026-09-16  
対象：要件定義書 v0.9 / 基本設計 v0.9 / UI/UX Specification v0.9
...
```text
1. mapping または既存ジョブがあるか？ → あれば、その状態に従う（接続状態は見ない）
2. mapping なし AND ジョブなし AND 現在も同期対象外 → Activity のみ削除
```

個別削除（§10.1）と全削除（§10.6）の両方でこの順序を使う。
...
| 順 | 条件 | 処理 |
|---:|---|---|
| 1 | `create` かつ `attempts = 0` かつ mapping なし | create ジョブを削除 |
| 2 | `create` かつ（`attempts > 0` または mapping あり） | delete へ置換 |
| 3 | `update` | delete へ置換 |
| 4 | `recreate` | delete へ置換 |
| 5 | ジョブなし かつ mapping あり | delete を作成 |
| 6 | ジョブなし かつ mapping なし | Activity のみ削除 |

**v0.8 までの表には次の2つの正常系が抜けていた。**
...
表の正本は基本設計 §10.1 とし、§10.6 では複製しない。
...
したがって **D-21（§10.1）の状態遷移を全 Activity へ一括適用する。**

| 状態 | 処理 |
|---|---|
| mapping あり | delete ジョブを作る |
| `create`、`attempts = 0` | ジョブごと削除 |
| `create`、`attempts > 0` | delete ジョブへ置換 |
| `update` / `recreate` | delete ジョブへ置換 |
| 同期対象外 | Activity のみ削除 |

D-41 の排他制御下で実行するため claim 中のジョブは存在せず、判定は `attempts` だけで足りる。
