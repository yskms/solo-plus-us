...
作成日：2026-09-16  
前提：要件定義書 v0.7 / 設計判断記録 v0.7  
v0.1 からの変更点と理由は **設計判断記録 v0.7** を参照する。
...
| 状態 | 処理 |
|---|---|
| mapping あり | delete ジョブを作る |
| `create` ジョブ、`attempts = 0` | **ジョブごと削除**（外部へ未到達が確実） |
| `create` ジョブ、`attempts > 0` | **delete ジョブへ置換** |
| `update` / `recreate` ジョブ | **delete ジョブへ置換** |
| 同期対象外 / HC 未接続 | Activity のみ削除 |
