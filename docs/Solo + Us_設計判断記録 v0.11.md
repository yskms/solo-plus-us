...
>   同内容を追記済み。**解消: 2026-09-21——D-34 の「§13.6 実装時の結論」
>   追記参照**）
...
> **2026-09-21 追記（§13.6 実装時の結論。実装中のレビューで、当初案
> 「ロジックは変更せず UI 注記のみで対応する」が §13.6 の主要ユース
> ケースを実際には救えないことが判明し、`recreateActivity` 自体の修正に
> 差し替えた。）**
>
> 実装当初は「ロジック無変更＋同意画面への UI 注記」を採用したが、
> レビューで次の点を指摘された：§13.6 の主要ユースケース（機種変更・
> 復旧）では復元先の Health Connect に対象レコードが1件も存在しない。
> Android 9〜13 ではこの場合 delete が確実に reject されるため（D-20
> 実測）、recreate は「大多数の環境で問題なく機能する」のではなく
> **全件・恒久的に**（リトライしても解消しない）失敗する。これは
> 当初案の前提が誤っていたことを意味する。
>
> **最終的に `recreateActivity`（`services/HealthConnectService.ts`）を
> 修正した：delete が `classifyError` で `UNKNOWN` に分類される失敗を
> した場合に限り、insert へ進む。** `PERMISSION_DENIED`/`UNAVAILABLE` は
> 従来通り即座に失敗を返す（insert を試みても同じ理由で失敗するだけで、
> 進む意味が無いため）。
>
> **「削除に失敗した場合は作成しない。外部に同じ記録を二重に作らない」
> という本 D-34 の当初の決定は、health_connect には適用されないことが
> 判明した。** この決定は provider が外部発行の ID でレコードを
> アドレッシングする場合（将来の HealthKit 等）を想定した、ジェネリックな
> 安全策として書かれていた。しかし v1 が実装する唯一の provider
> （health_connect）は §9.4 の通り `clientRecordId`（= `activity.id`、
> クライアント側で決定的に発行）でアドレッシングし、同じ clientRecordId
> への insert は version の大小を HC 側が自動的に処理する upsert として
> 振る舞う（`HealthConnectService.ts` の `toRecord`/`upsertActivity`
> 参照）。したがって delete が失敗していても insert は新規レコードを
> 作らず、二重レコードという当初の懸念は health_connect では構造的に
> 発生しない。
>
> 唯一残る理論的リスクは、delete が「存在しない」以外の一過性の理由で
> `UNKNOWN` 分類の失敗をし、かつ対象が実際により高い
> `clientRecordVersion` で HC 上に存在している場合、insert が黙って
> 無視され（HC 側の version 優先ルール）、ジョブが誤って成功扱いになる
> ことである。ただし：
>
> - これは通常の `update` ジョブでも起こりうる既知のリスクと同種であり、
>   recreate 特有の新しいリスクではない
> - `delete`/`insert` は同一関数内で連続して呼ばれるため、delete が
>   一過性の理由（Binder 切断等）で失敗した場合、直後の insert も同じ
>   理由で失敗する可能性が高い——その場合はジョブ全体が失敗し通常の
>   リトライに乗る（安全側）
> - D-20 で実測された Android 9〜13 の「存在しない」ケースはこの
>   「対象が実際に存在している」条件を満たさない
>
> このリスクが顕在化した場合の結果（HC 側が古いまま）は、案2・3
> （ロジック不変のまま UI 注記のみ、または §13.6 自体を先送り）を採った
> 場合に Android 9〜13 の**全ユーザーに確定的に**起きていた結果と同じで
> あり、悪化させる経路は無い。
>
> 実装は `services/HealthConnectService.ts` の `recreateActivity` 1関数
> に閉じており、`classifyError`・`deleteActivityRecord`・
> `syncJobPlanner.planForDelete`（§9.7 の delete ジョブ経路）は無変更。
> OS バージョンによる分岐、エラーメッセージ文字列への依存のいずれも
> 追加していない——D-20 が守ろうとした2つの原則は維持されている。
>
> 同意画面（`app/settings/data.tsx` の `offerResync` ステップ）の文言も、
> 「一部の記録が長時間かかる場合がある」という当初案の注記から、通常の
> 同期失敗と同程度の軽い注記に戻した——主要ユースケースでの恒久的失敗が
> 解消されたため、大掛かりな警告は不要と判断した。
>
> なお、`app/settings/health-connect.tsx` に「Sync everything to Health
> Connect」という恒久的な再同期導線を追加した（別のレビュー指摘、
> `services/HealthSyncResyncService.ts` の doc comment 参照）。これは
> `offerResync` を逃した場合の救済導線であり、本項の決定とは独立した
> 変更である。
>
> **2026-09-21 追記2（2巡目のレビューを受けての追加修正）**
>
> 恒久的な入口を追加した結果、次の2点が新たに問題になり、対応した：
>
> 1. **`sync_state = 'synced'` の mapping を対象外にしていたため、Health
>    Connect アプリ側で直接削除された記録（D-20 が実機検証した経路その
>    もの）をこの一括操作では二度と救済できなかった。** ローカルは READ
>    権限を持たない（D-12/D-20）ため「synced のままだが実際には HC 側に
>    無い」を判別できない。`recreate` は常に安全（本項の性質）なので、
>    `sync_state = 'synced'` は対象から外さないことにした——対象外のまま
>    なのは `declined`/`uncertain`（D-51、利用者の明示的な「同期しない」
>    選択）のみ。これにより「Sync everything」の確認文言（"replacing
>    what's in Health Connect for any that already exist there"）も
>    実態と一致するようになった。
> 2. **この恒久的な入口は §13.6 が本来規定する「Import 後の再同期」を
>    超えて使える**（例：HC を初めて ON にした直後に押せば、過去の全
>    履歴を一括送信できる）。§13.1「既定 OFF・明示同意制」自体には
>    反しないが、センシティブなデータをまとめて外部へ送る操作である
>    ため、送信前に対象件数を数えて確認ダイアログに出す
>    `countPendingResync`（`resolveResyncTargets` を `queueResync` と
